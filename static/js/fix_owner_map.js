...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  const storedLat = {{ selected_pg.latitude|default:"null" }};
  const storedLng = {{ selected_pg.longitude|default:"null" }};
  
  if (storedLat !== null && storedLng !== null) {
    // Use stored coordinates
    initMapWithCoords(storedLat, storedLng, rawAddress);
    return;
  }
  ...
  try {
    ...
    if (data.length && data[0]) {
      ...
      initMapWithCoords(lat, lon, rawAddress);
...
function initMapWithCoords(lat, lng, address) {
  const mapContainer = document.getElementById('propertyMap');
  mapContainer.style.height = '360px';
  mapContainer.innerHTML = '';
  
  const map = L.map('propertyMap').setView([parseFloat(lat), parseFloat(lng)], 16);
  ...
  L.marker([parseFloat(lat), parseFloat(lng)], {icon: pgIcon}).addTo(map)
    .bindPopup(`
      <div style="max-width:250px;padding:12px;">
        <h6 style="margin:0 0 8px 0;font-weight:700;color:#1e293b;">{{ selected_pg.title }}</h6>
        <p style="margin:4px 0;font-size:13px;color:#64748b;">📍 ${address}</p>
